...
if (saveEditInformation) {
    ...
    saveEditInformation.addEventListener(
        ...
        async function () {
            ...
            if (pegawaiFoto && pegawaiFoto.files.length > 0) {

                pegawaiMessage.textContent =
                    "📤 Mengupload foto...";

                fotoUrl =
                    await uploadFotoPegawai(
                        pegawaiFoto.files[0]
                    );
...
function renderPegawai(data = pegawaiData) {
    ...
    <div class="admin-pegawai-photo">
        ...
        ${pegawai.foto
              ...
              `
                : `
                <div class="admin-pegawai-photo-placeholder">
                    👤
                </div>
              `
            }
    ...
    pegawaiSaveButton.addEventListener(
        ...
        async function () {
            ...
            if (pegawaiFoto && pegawaiFoto.files.length > 0) {
                ...
                fotoUrl = await uploadFotoPegawai(
                    pegawaiFoto.files[0]
                );
            ...
            if (pegawaiFotoPreview) {
                pegawaiFotoPreview.innerHTML =
...
let editPegawaiId = null;

const editPegawaiModal =
    document.getElementById("editPegawaiModal");

const editPegawaiModalOverlay =
    document.getElementById("editPegawaiModalOverlay");

const editPegawaiModalClose =
    document.getElementById("editPegawaiModalClose");

const editPegawaiCancelButton =
    document.getElementById("editPegawaiCancelButton");

const editPegawaiSaveButton =
    document.getElementById("editPegawaiSaveButton");

const editPegawaiMessage =
    document.getElementById("editPegawaiMessage");

const editPegawaiNama =
    document.getElementById("editPegawaiNama");

const editPegawaiNip =
    document.getElementById("editPegawaiNip");

const editPegawaiJabatan =
    document.getElementById("editPegawaiJabatan");

const editPegawaiDivisi =
    document.getElementById("editPegawaiDivisi");

const editPegawaiEmail =
    document.getElementById("editPegawaiEmail");

const editPegawaiNoHp =
    document.getElementById("editPegawaiNoHp");

const editPegawaiFoto = document.getElementById("editPegawaiFoto");

const editPegawaiFotoPreview =
    document.getElementById("editPegawaiFotoPreview");

if (editPegawaiFoto) {

    editPegawaiFoto.addEventListener(
        "change",
        function () {

            const file =
                editPegawaiFoto.files[0];
            ...
            reader.onload = function (event) {
                ...
                editPegawaiFotoPreview.innerHTML = `
                    <img
...
const editPegawaiStatus =
    document.getElementById("editPegawaiStatus");


function openEditPegawai(id) {

    const pegawai =
        pegawaiData.find(function (item) {
    ...
    editPegawaiNama.value =
        pegawai.nama || "";

    editPegawaiNip.value =
        pegawai.nip || "";

    editPegawaiJabatan.value =
        pegawai.jabatan || "";

    editPegawaiDivisi.value =
        pegawai.divisi || "";

    editPegawaiEmail.value =
        pegawai.email || "";

    editPegawaiNoHp.value =
        pegawai.no_hp || "";

    editPegawaiStatus.value =
        pegawai.status || "Aktif";

    // Tampilkan foto lama
    if (editPegawaiFoto) {
        editPegawaiFoto.value = "";
    }

    if (editPegawaiFotoPreview) {
    ...
    editPegawaiMessage.textContent = "";

    editPegawaiModal.classList.add("show");
...
function closeEditPegawai() {

    editPegawaiModal.classList.remove("show");
...
if (editPegawaiModalClose) {
    editPegawaiModalClose.addEventListener(
        "click",
        closeEditPegawai
    );
}
...
if (editPegawaiCancelButton) {
    editPegawaiCancelButton.addEventListener(
        "click",
        closeEditPegawai
    );
}
...
if (editPegawaiModalOverlay) {
    editPegawaiModalOverlay.addEventListener(
        "click",
        closeEditPegawai
    );
}
...
if (editPegawaiSaveButton) {

    editPegawaiSaveButton.addEventListener(
        "click",
        async function () {

            if (!editPegawaiId) return;

            const nama =
                editPegawaiNama.value.trim();
            ...
            if (!nama) {
                ...
                editPegawaiMessage.style.color =
                    "#c0392b";

                return;
            }

            editPegawaiSaveButton.disabled = true;

            editPegawaiSaveButton.textContent =
                "Menyimpan...";


            // ==================================
            // CARI DATA PEGAWAI LAMA
            // ==================================

            const pegawaiLama =
                pegawaiData.find(function (item) {
                    return String(item.id) === String(editPegawaiId);
                });

            const fotoLama =
                pegawaiLama ? pegawaiLama.foto : null;
            ...
            const updateData = {
                ...
                nip:
                    editPegawaiNip.value.trim(),

                jabatan:
                    editPegawaiJabatan.value.trim(),

                divisi:
                    editPegawaiDivisi.value.trim(),

                email:
                    editPegawaiEmail.value.trim(),

                no_hp:
                    editPegawaiNoHp.value.trim(),
            ...
            if (
                editPegawaiFoto &&
                editPegawaiFoto.files.length > 0
            ) {
                ...
                if (!fotoUrl) {
                    ...
                    editPegawaiSaveButton.disabled =
                        false;
            ...
            // ==================================
            // HAPUS FOTO LAMA
            // Hanya jika foto baru digunakan
            // ==================================

            if (
                updateData.foto &&
                fotoLama &&
                updateData.foto !== fotoLama
            ) {

                await deleteFotoPegawai(
                    fotoLama
                );
            }
...
document.addEventListener("click", async function (event) {
    ...
    const pegawai = pegawaiData.find(function (item) {
        return String(item.id) === String(id);
    });
...
function updatePegawaiStatistics() {

    const totalElement =
        document.getElementById("totalPegawai");

    const aktifElement =
        document.getElementById("pegawaiAktif");

    const tidakAktifElement =
        document.getElementById("pegawaiTidakAktif");

    const persentaseElement =
        document.getElementById("pegawaiPersentaseAktif");
    ...
    const aktif =
        pegawaiData.filter(function (pegawai) {
            return pegawai.status === "Aktif";
        }).length;

    const tidakAktif =
        pegawaiData.filter(function (pegawai) {
            return pegawai.status === "Tidak Aktif";
        }).length;


    // Hitung persentase pegawai aktif
    const persentaseAktif =
        total > 0
            ? Math.round((aktif / total) * 100)
            : 0;
...
const detailPegawaiModal =
    document.getElementById("detailPegawaiModal");

const detailPegawaiModalOverlay =
    document.getElementById("detailPegawaiModalOverlay");

const detailPegawaiModalClose =
    document.getElementById("detailPegawaiModalClose");


function openDetailPegawai(id) {
    ...
    const detailFoto =
        document.getElementById("detailPegawaiFoto");

    if (detailFoto) {

        if (pegawai.foto) {
            detailFoto.src = pegawai.foto;
            detailFoto.style.display = "block";
        } else {
            detailFoto.removeAttribute("src");
            detailFoto.style.display = "none";
        }

    }

    document.getElementById("detailPegawaiNama").textContent =
        pegawai.nama || "-";

    document.getElementById("detailPegawaiStatus").textContent =
        pegawai.status || "Aktif";

    document.getElementById("detailPegawaiNip").textContent =
        pegawai.nip || "-";

    document.getElementById("detailPegawaiJabatan").textContent =
        pegawai.jabatan || "-";

    document.getElementById("detailPegawaiDivisi").textContent =
        pegawai.divisi || "-";

    document.getElementById("detailPegawaiEmail").textContent =
        pegawai.email || "-";

    document.getElementById("detailPegawaiNoHp").textContent =
        pegawai.no_hp || "-";

    detailPegawaiModal.classList.add("show");
...
function closeDetailPegawai() {

    detailPegawaiModal.classList.remove("show");

    document.body.style.overflow = "";
}


if (detailPegawaiModalClose) {

    detailPegawaiModalClose.addEventListener(
        "click",
        closeDetailPegawai
    );
...
if (detailPegawaiModalOverlay) {

    detailPegawaiModalOverlay.addEventListener(
        "click",
        closeDetailPegawai
    );
...
async function uploadFotoPegawai(file) {
...
async function deleteFotoPegawai(fotoUrl) {
